...
public class TicTacToe extends GameSearch {
    ...
    public float positionEvaluation(Position p, boolean player) {
        int count = 0;
        TicTacToePosition pos = (TicTacToePosition)p;//TicTacToePosition是个类:表明该游戏的9个位置，并标明位置有三种情况:BLANK、HUMAN、PROGRAM
        for (int i=0; i<9; i++) {
            if (pos.board[i] == 0) count++;//总共9个位置=3x3 0表示是blank
        }
        count = 10 - count;
        // prefer the center square:
        float base = 1.0f;
	//这里检查pos.board[4]是为什么呢，是因为4是3*3的棋盘的核心位置
        if (pos.board[4] == TicTacToePosition.HUMAN &&//player==1代表human
            player) {
            base += 0.4f;
        }
        ...
        float ret = (base - 1.0f);//因为初始化时就是1.0f
        if (wonPosition(p, player))  {//如果是winposition，就给自己加权重
    ...
    public Position [] possibleMoves(Position p, boolean player) {
        ...
        TicTacToePosition pos = (TicTacToePosition)p;
        int count = 0;
        for (int i=0; i<9; i++) if (pos.board[i] == 0) count++;//数blank的位置数
        if (count == 0) return null;
        Position [] ret = new Position[count];
        count = 0;
        for (int i=0; i<9; i++) {
            if (pos.board[i] == 0) {//列举出所有的nextmove可能性，即列举出所有:在原有的position(序列)基础上加了一个位置的可能情况
    ...
    public Position makeMove(Position p, boolean player, Move move) {
        ...
        pos2.board[m.moveIndex] = pp;//表示走了那一步
        return pos2;
    }
